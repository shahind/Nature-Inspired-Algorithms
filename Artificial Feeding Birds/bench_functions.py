...
def sphere(p):
  r = 0
  for j in range(len(p)): r += p[j] * p[j]
  return r
...
def rastrigin(p):
  ...
  for i in range(len(p)):
    r += (p[i] ** 2) - 10.0 * math.cos( 2.0 * math.pi * p[i] ) + 10.0
  return r

def rosenbrock(p):
  return 100.0 * ((p[1] - (p[0] ** 2)) ** 2) + ((p[0] - 1.0) ** 2)

def eggholder(p):
  return -(p[1] + 47) * math.sin(math.sqrt(abs( p[0] / 2.0 + p[1] + 47 ))) - p[0] * math.sin(math.sqrt(abs( p[0] - (p[1] + 47) ))) + 959.640662720851

def pics4(p):
  x, y = p
  return math.exp(-(x-4)**2 - (y-4)**2) + math.exp(-(x+4)**2 - (y-4)**2) + 2 * (math.exp(- x**2 - y**2) + math.exp(- x**2 - (y+4)**2))

def f__(p):
  d1, d2, r, L = p
  
  if -d1 + 0.0193  * r > 0: return 99999999.0
  if -d2 + 0.00954 * r > 0: return 99999999.0
  if -math.pi * r * r * L - (4.0 / 3.0) * math.pi * r * r * r + 1296000 > 0: return 99999999.0
  if L - 240 > 0: return 99999999.0
  
  return 0.6224 * d1 * r * L + 1.7781 * d2 * r * r + 3.1661 * d1 * d1 * L + 19.84 * d1 * d1 * r
...
def _xor6_output(p, inputs, act_func):
  hidden1 = act_func(inputs[0] * p[0] + inputs[1] * p[1])
  hidden2 = act_func(inputs[0] * p[2] + inputs[1] * p[3])
  return act_func(hidden1 * p[4] + hidden2 * p[5])
def xor6_sig(p):
  try:
    mean_square_error = 0.0
    for inputs, expected_output in _XOR_RULES:
      mean_square_error += (expected_output - _xor6_output(p, inputs, _sigmoid)) ** 2
    mean_square_error = mean_square_error / len(_XOR_RULES)
...
def xor6_sin(p):
  try:
    mean_square_error = 0.0
    for inputs, expected_output in _XOR_RULES:
      mean_square_error += (expected_output - _xor6_output(p, inputs, math.sin)) ** 2
    mean_square_error = mean_square_error / len(_XOR_RULES)
    return mean_square_error
  except OverflowError:
    return 999.0
...
def _xor9_output(p, inputs):
  hidden1 = _sigmoid(inputs[0] * p[0] + inputs[1] * p[1] + p[6])
  hidden2 = _sigmoid(inputs[0] * p[2] + inputs[1] * p[3] + p[7])
  return _sigmoid(hidden1 * p[4] + hidden2 * p[5] + p[8])
def xor9(p):
  try:
    mean_square_error = 0.0
    for inputs, expected_output in _XOR_RULES:
      mean_square_error += (expected_output - _xor9_output(p, inputs)) ** 2
    mean_square_error = mean_square_error / len(_XOR_RULES)
    return mean_square_error
  except OverflowError:
    return 999.0
...
class TravelingSalesmanProblem(object):
  def __init__(self, best, dataset):
    self.best      = best
    self.distances = {}
    i = 0
    for line in dataset.split("\n"):
      ...
      for cell in line.split():
        self.distances[i, j] = float(cell)
        j += 1
      i += 1
    self.nb_city = i
    self.cities = list(range(i))
    
  def __call__(self, x):
    d = 0
    for i in range(len(x)): d += self.distances[x[i - 1], x[i]]
    return d



wan14_tsp = TravelingSalesmanProblem(30.8785, """ 0.0 1.6599999999999966 5.077203954934244 6.519148717432358 8.83386665056701 5.530226035163483 4.104436623947311 0.7543208866258478 1.291239714383043 3.1522690240523623 1.281405478371309 5.0756871455991055 3.115204648173216 3.9378801403801003
...
fri26_tsp = TravelingSalesmanProblem(937, """   0  83  93 129 133 139 151 169 135 114 110  98  99  95  81 152 159 181 172 185 147 157 185 220 127 181
...
att48_tsp = TravelingSalesmanProblem(10628, """         0      4727      1205      6363      3657      3130      2414       563       463      5654      1713      1604      2368      2201      1290      1004      3833      2258      3419      2267      2957       720      1700      5279      2578      6076      3465      2654      3625      3115      1574      3951      1748      2142      6755      2383      3306      1029      3530       825      2188      4820      3489      1947      6835      1542      2379      3744
